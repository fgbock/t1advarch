...
void multiplica(int m, int n, int **matA, int **matB, int **matC, int **matD){
	...
	for (i=0;i<m;i++){
		for(j=0;j<n;j++){
			for(k=0; k<p; k++){
				matC[i][j]+= matA[i][k*8]*matD[j][k*8];
				matC[i][j]+= matA[i][k*8+1]*matD[j][k*8+1];
				matC[i][j]+= matA[i][k*8+2]*matD[j][k*8+2];
				matC[i][j]+= matA[i][k*8+3]*matD[j][k*8+3];
				matC[i][j]+= matA[i][k*8+4]*matD[j][k*8+4];
				matC[i][j]+= matA[i][k*8+5]*matD[j][k*8+5];
				matC[i][j]+= matA[i][k*8+6]*matD[j][k*8+6];
				matC[i][j]+= matA[i][k*8+7]*matD[j][k*8+7];
			}
		}
	}
}
